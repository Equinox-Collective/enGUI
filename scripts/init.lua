--- START OF FILE res/sysgui/init.lua ---
...
function on_tick(dt)
    ...
    if dragging_win then
        if my < 5 then
            drawRect(2, 24, sw-4, sh-70, 0x5C6370) -- Full screen layout preview
        elseif mx < 5 then
            drawRect(2, 24, math.floor(sw/2)-2, sh-70, 0x5C6370) -- Left layout preview
        elseif mx > sw - 5 then
            drawRect(math.floor(sw/2)+2, 24, math.floor(sw/2)-4, sh-70, 0x5C6370) -- Right layout preview
        end
    end
    ...
    if not mdown and dragging_win then
        -- Process Aero Snap drop trigger
        if my < 5 then
    ...
    -- Smooth Fish-Eye Dock Icons Layout
    ...
    for idx, win in ipairs(windows) do
        if win.active and not win.borderless then
            ...
            -- Fish-eye scale calculation based on mouse distance
            ...
            -- Active dot indicator under running dock apps
...
--- END OF FILE res/sysgui/init.lua ---