...
-- "под длину звука" + 0.8 c растворение) и СИНХРОННО грузил 846 КБ WAV с диска
-- через ATA PIO, тормозя первый кадр рабочего стола.
-- FAST_BOOT: короткий брендинг и без блокировки на звуке.
-- ЗВУК ЗАПУСКА: теперь воспроизводится из C (api_try_boot_sound в main.c) ровно
-- один раз, как только готова звуковая карта AC'97 (она инициализируется в
-- фоновом потоке при быстрой загрузке). Поэтому PLAY_BOOT_SOUND здесь ВЫКЛЮЧЕН,
-- чтобы звук не запускался дважды. Включается/выключается дефайном
-- BOOT_SOUND_ENABLED в api_gui.c.
